    ...
    <GeneralLayout
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="container grid grid-cols-1 lg:grid-cols-3 gap-4 py-4">
          <div className="col-span-2 flex flex-col justify-start order-2 lg:order-1">
            {reviews
              .filter((item) => item.locationId === locationId)
              .map((item, index) => {
          ...
          <div className="col-span-1 order-1 lg:order-2 grid grid-cols-4 lg:grid-cols-2 gap-4">
            {reviews
              .filter((item) => item.locationId === locationId)
                  ...
                  <div key={index} className="w-full aspect-square overflow-hidden rounded relative">
                    ...
                    {index === 3 && (
                      <div className="absolute top-0 left-0 w-full h-full bg-black/50 grid place-items-center z-[1]">
                        <p className="text-center uppercase text-white text-[0.5rem] md:text-base truncate">View More</p>
                      </div>
                    )}
                  </div>
                );
